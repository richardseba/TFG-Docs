...
\begin{document}
	...
	\lettrine[lines=3]{T}{his} project has its origins in the internship that I did at the Computer Vision Center (CVC) during the summer of 2017. During my internship I developed a basic video-see-through viewer for a head mounted display (HMD) developed in the CVC. Along with the development we discovered that the main concern that our prototypes faced during our user testing sessions was the feeling of dizziness and motion sickness produced by the headset. Hence the motivation for this project is reduce the adverse physical reactions that it produces to the users \cite{disconfortReview}, \cite{unpublishCVC} and continue improving the viewer adding new modules while keeping its expandability. It has to be said that this research project is focused mainly on the software, the prototypes and other hardware aspects will be out of our concerns and will be developed by CVC researchers. A communication channel though is opened to discuss about the development of the whole project. 
	
	The head mounted displays first appeared in 1965 when Ivan Sutherland developed the first HMD called "the sword of Damocles" \cite{hdmSutherland}, it established the ground for further development in the field. The development of these technologies has grown in the last years mainly centered in the video-games field. Some examples, are the Oculus \cite{web:oculus} or the HTC Vive \cite{web:vive}. These products are virtual reality headsets, therefore they are only capable of showing computer generated scenes. The developed project prototypes differ in this matter because they can also show the real world using two integrated cameras. This kind of HMDs are called video-see-through, the current version of the prototype can be seen in Figure \ref{fig:proto}.
	...
	First, we will develop a software that will be capable of capture stream from a stereo camera and display it in real time. The software will show one camera stream on each half of the screen, also the HMD will enable only to each eye to see one half of the screen, all of this will simulate the stereo effect (creating a depth effect). Another goal is to prepare this software to be easily expandable with new modules in the future.
	...
	\begin{itemize}
		\item To perform the testing, 3 ophthalmological patterns, Figure \ref{fig:snellen}, will be set in 3 different locations, one at reading distance, other at close distance, like the pc screen; and other far away at the wall of the room. 
...
\begin{figure}
	\centering
	\includegraphics[width=1\linewidth]{img/Snellenchart_es-1}
	\caption{Ophthalmological pattern used in the user testing sessions from task \ref{obj:acc:preset}. The pattern was downloaded from \cite{web:snellen}.}
	\label{fig:snellen}
\end{figure}
	...
	This problem can be resolved in two ways, using a disparity map only to create the DoF and using Neural Network to aid the creation of it.
	...
	During the firsts weeks of work some problems arose, the main issue was that in order to integrate a system capable of generating depth map image, it was required to undistort the image pair captured. This is because each optical lens of each camera introduces a distortion to the image captured. In addition, as the lenses are not exactly equal and they are not exactly aligned, a stereo calibration system was required to produce the undistortion. This stage of the project was split in four parts.
	...
	\end{itemize}
	In this case, the first option will be selected, but if needed the second solution can also be used. In Table \ref{tab:depthmap} can be observed the current performance of the depth map generator. As well as the times, in the Figure \ref{fig:depthmap} we can see a full resolution disparity map and the original image. In this example is visible that the disparity is far from perfect, that is because there is still some calibration that can be done in the LIBELAS parameters. Also, is worth mention the extremely high accuracy needed in the calibration phase to obtain a correct depth map, any minor change in the camera setup, can give inaccurate depth maps.
	...
	Related with the task \ref{obj:dispmap}, a color map has been added to improve the visibility of the depth. This was added  because for humans is easier to see a gradient of colors rather than levels of gray.  
	...
	First, after the finishing the user testing session, we notice that users felt more comfortable in general, this is mainly due to the improvements in the prototype itself and because the improvement on the viewer that now uses the full screen instead of leaving black margins. Having said that, there is still work to do to include a dynamic change in the vergence using the distances obtained by the depth map.
	Second, after the results of the first user testing session, we concluded that further analysis and therefore more user testing must be done to prove that the pattern that we discovered is true. If our theory confirms, we will start working on the dynamic vergence. 
	...
	\begin{landscape}
		...
		\begin{figure}[b!]
			\centering
			\includegraphics[width=1\linewidth]{img/gantt3}
			\caption{Gantt diagram of the project. Note that blue tasks are remaining tasks, grey task are done tasks, red tasks are tasks not done in the scheduled time, yellow tasks are rescheduled tasks, black tasks are task that will not be completed and green tasks are tasks that probably will not be completed due to time constrains }
			\label{fig:gantt}
		\end{figure}
