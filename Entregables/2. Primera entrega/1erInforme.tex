...
\begin{document}
	...
	The performance of the depth map generation is also key, as a slow image generation will give the system great latency and that could limit the uses cases of the system. \\
	...
	\end{itemize}
	In this case, the first option will be selected, but if needed the second solution can also be used. In the table \ref{tab:depthmap} can be seen the current performance of the depth map generator. As well as the times, in the figure \ref{} we can see the full resolution disparity map of the image \ref{}.

	\begin{table}
		\centering
		\begin{tabular}{|c|c|c|c|}
			\hline 
			Seconds b/w frames & Fps & Width & Height \\ 
			\hline 
			1.98 & 0.50 & 1600 & 1200 \\ 
		...
		\end{tabular} 
	\caption{Frames per second (Fps) for the generation of the depth map for each image resolution }
	\label{tab:depthmap}
	\end{table}

	%Insert photo of map here!
	...
	\begin{itemize}
		\item Calibration development: To develop the calibration and undistortion, the OpenCV library \cite{web:opencv} was used as it has some calibration functions already implemented and is a well known library.
		\item Depth map development: As it is explained in \ref{subsec:vergence}, LIBELAS library was selected. One of the main reasons for this selection was the performance. !!!!!!!!!!!!!!!!!
