...
\begin{document}
	...
	One of the issues had was that LIBELAS was not able to produce real time processing, as is explained in \ref{sec:resizeperformance}. For that reason, a video recording and processing modules were also implemented.  
	...
	\begin{table}
		...
		\end{tabular}
		\caption{Computing time of LIBELAS and all the processing module in milliseconds.}
		\label{my-label}
	\end{table}
	...
	explicar mean mola y min va, max no, porque no va el max etc
	...
	\begin{itemize}
		...
		\item Optimization of the software can be done to allow to use it with a better experience on smaller and less powerful devices. 
		...
		\item Despite these comments, the transition module did not give as good feedback as the other, users said that it was slow and uncomfortable to see. Therefore an improvement on this matter can be done, and further user testing can be done to verify if a transition is needed or is better to change the setting without any.
	\end{itemize}
