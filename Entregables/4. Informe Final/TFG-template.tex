...
\begin{document}
	...
	\subsection{Libelas}
	 ...
	 One of the problems faced by this library, and in general by depth map stereo matchers, is the existence of low textured areas on the image, as will be seen in \ref, these regions usually end up without disparity values inducing issues in further processing. These regions along with occluded ones and calibration faulty areas are usually set to negative values by Libelas itself, therefore can be easily detected.  
	 ...
	 \begin{itemize}
	 	\item The first idea is to apply a ROI in the center of the images after the distortion, that way the image requires less computing power to process, keeping all the detail of the center where usually the user will be looking at. 
	 	\item Other idea is to make a down sampling of the images before using them on LIBELAS. This also decreases the computing power required to process the images but also decreases the detail of the image, potentially affecting the quality of the depth map.  
 	 \end{itemize}
	 In \ref can be seen a comparison between both methods. 
