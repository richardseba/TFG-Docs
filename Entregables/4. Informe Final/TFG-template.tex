...
\begin{document}
	...
	\vspace{1cm}
		\begin{subfigure}[t]{0.5\textwidth}
			\centering
			\includegraphics[width=\linewidth]{img/rectifyed.png}
			\caption{Ractification of the images in \ref{fig:rec:regular}.}
			\label{fig:rec:ractified}
		\end{subfigure}
		\caption{Figure \ref{fig:rec:regular} is an example one image pair as it is captured from the cameras, the distortion produced by the optic of the cameras can be seen. In these images the objects cannot be found on the same Y axis. In Figure \ref{fig:rec:ractified} the same images from \ref{fig:rec:regular} have been rectified using the calibration module. The distortion produced by the lenses of the camera is gone and the objects can be found in the same Y axis. }
		\label{fig:rec}
	\end{figure}
